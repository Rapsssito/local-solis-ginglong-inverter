...
class LoggerServer:
    ...
    async def __handle_connection(self, reader, writer):
        ...
        _LOGGER.debug(' '.join(format(x, '02x') for x in data) + ' from ' + str(addr))
        response = _mock_server_response(data)
        writer.write(response)
        writer.close()
        # Convert data to dict and pass to on_data
        if data[4] == 0x42:
            _LOGGER.debug(' '.join(format(x, '02x') for x in response) + ' to ' + str(addr))
            self.on_data(_extract_data(data))

    async def start_server(self):
        self.server = await asyncio.start_server(self.__handle_connection, '0.0.0.0', self.port)
